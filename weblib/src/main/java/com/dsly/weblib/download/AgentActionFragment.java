...
//        mAction = null;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            X5LogUtils.i("savedInstanceState:" + savedInstanceState);
            return;
        }
        isViewCreated = true;
        runAction();
    ...
    private void requestPermission(Action action) {
        ...
        if (X5WebUtils.isEmptyCollection(permissions)) {
            resetAction();
            return;
        }
    ...
    private void recordVideo() {
        try {
            ...
            File mFile = X5WebUtils.createVideoFile(this.getActivity());
            if (mFile == null) {
                mAction.getChooserListener().onChoiceResult(REQUEST_CODE, Activity.RESULT_CANCELED, null);
                resetAction();
                return;
            }
            Intent intent = X5WebUtils.getIntentVideoCompat(getActivity(), mFile);
            // 指定开启系统相机的Action
            mAction.setUri((Uri) intent.getParcelableExtra(EXTRA_OUTPUT));
            this.startActivityForResult(intent, REQUEST_CODE);
        } catch (Throwable ignore) {
