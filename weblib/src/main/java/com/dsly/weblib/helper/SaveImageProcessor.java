...
*/
package com.dsly.weblib.helper;
...
import com.dsly.weblib.utils.OkHttpUtils;
import com.dsly.weblib.utils.ToastUtils;
import com.tencent.smtt.sdk.WebView;
import com.dsly.weblib.utils.WebFileUtils;
    ...
    private void saveImage(final Context context, final WebView webView) {
        ...
        if (imageUrl == null) {
            ...
            try {
                ...
                ToastUtils.showLong("保存图片成功"+imagePath);
            } catch (IOException e) {
            ...
            OkHttpUtils.downloadFile(context, imageUrl, imageFile, new OkHttpUtils.FileCallback() {
                ...
                public void fail(int i, Exception e) {
                    webView.post(new Runnable() {
                        @Override
                        public void run() {
                            ToastUtils.showLong("请稍后再试，请链接网络");
                        }
                    });
                }
            });
