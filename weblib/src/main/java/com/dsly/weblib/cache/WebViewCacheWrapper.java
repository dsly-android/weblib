...
public class WebViewCacheWrapper implements WebViewRequestClient {
    ...
    private WebResourceResponse interceptRequest(String url, Map<String, String> headers) {
        ...
        if (isEnableAssets()) {
            ...
            if (inputStream != null) {
                Log.d("cache", "WebViewCacheWrapper---interceptRequest1--" + String.format("from assets: %s", url));
        ...
        try {
            ...
            if (cacheRes != null) {
                //应用缓存
                Log.d("cache", "WebViewCacheWrapper---interceptRequest2--" + String.format("from cache: %s", url));
            } else {
                //没有缓存
                Log.d("cache", "WebViewCacheWrapper---interceptRequest3--" + String.format("from server: %s", url));
            }
    ...
    private String getOriginUrl(String referer) {
        ...
        try {
            ...
            Log.i("cache", "WebViewCacheWrapper---getOriginUrl--" + ou);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return ou;
